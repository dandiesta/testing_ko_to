      ...
      <td colspan="2">
        <div class="row app-list-item-info">
          ...
          <div class="col-xs-12 col-sm-6">
            ...
            {{ date('Y-m-d H:i',strtotime(($app->upload_time)?$app->upload_time:$app->created_at)) }}
@if($app->app_install_date && $app->upload_time > $app->app_install_date)
            <span class="label label-success">UPDATE</span>
@elseif(strtotime($app->upload_time)>strtotime('yesterday'))
...
@endif
        ...
        <div class="row xs-buttons visible-xs">
          ...
          <div class="btn-group btn-group-sm notification-toggle" data-app-id="{{ $app->id }}">
            <button class="btn btn-default{{ $app->notify_setting ?' active':'' }}" value="1">ON</button>
            <button class="btn btn-default{{ $app->notify_setting ?'':' active' }}" value="0">OFF</button>
          </div>
